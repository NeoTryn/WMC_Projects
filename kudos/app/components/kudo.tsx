...
export function Kudo({ profile, kudo }: { profile: Profile; kudo: Partial<IKudo> }) {
  return (
    <div
      className={`flex ${
        backgroundColorMap[kudo.backgroundColor || 'RED']
    ...
    >
      ...
      <div className="flex flex-col">
        <p className={`${colorMap[kudo.textColor || 'WHITE']} font-bold text-lg whitespace-pre-wrap break-all`}>
          {profile.firstName} {profile.lastName}
        </p>
        <p className={`${colorMap[kudo.textColor || 'WHITE']} whitespace-pre-wrap break-all`}>{kudo.message}</p>
      </div>
      <div className="absolute bottom-4 right-4 bg-white rounded-full h-10 w-10 flex items-center justify-center text-2xl">
        {emojiMap[kudo.emoji || 'THUMBSUP']}
      </div>
    </div>
  )
}